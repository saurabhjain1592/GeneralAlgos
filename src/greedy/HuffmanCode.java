...
public class HuffmanCode<T> {
	...
	private HuffmanNode createHuffmanCode(T a[], double p[],
			ArrayList<ArrayList<Integer>> mnAl, double q[], int l1, int l2,
			int s2) {
		if (l2 > s2) {
			if (l1 < p.length - 2) {
				int x = l1, y = l1 + 1;
	...
	private HuffmanNode getHuffmanNode(T a[],
			ArrayList<ArrayList<Integer>> mnAl, double p[], int l, int i) {
		if (i == mnAl.get(l).size() - 2) {
			HuffmanNode hn = new HuffmanNode();
			hn.left = new HuffmanNode(a[mnAl.get(l).get(i)], p[mnAl.get(l).get(
					i)]);
			hn.right = new HuffmanNode(a[mnAl.get(l).get(i + 1)], p[mnAl.get(l)
					.get(i + 1)]);
			return hn;
		} else {
			HuffmanNode hn = new HuffmanNode();
